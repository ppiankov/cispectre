...
```
brew install ppiankov/tap/cispectre
```

Or via Go:

```
go install github.com/ppiankov/cispectre/cmd/cispectre@latest
```
...
Scans CI/CD pipelines for security findings.
...
- Does not remediate or modify CI/CD pipelines — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated CI/CD pipelines monitoring — point-in-time security audit only
...
```bash
cispectre scan --output json | jq '.summary'
cispectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
